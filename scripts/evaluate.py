...
def gen_images(root_dir, x, y, x_names, y_names, args,device,save = True):
    g_x_y = Generator(args)
    g_x_y.load_state_dict(torch.load(os.path.join(root_dir,"g_x_y.pt"), map_location=device))
    with torch.no_grad():
...
def gen_images_VAE(root_dir, x, y, x_names, y_names, args,device,save = True):
    ...
    VAE_human = VAE()
    VAE_cartoon = VAE()
    VAE_human.load_state_dict(torch.load(os.path.join(root_dir,"vaehuman.pt"), map_location=device))
    VAE_cartoon.load_state_dict(torch.load(os.path.join(root_dir,"vaecartoon.pt"), map_location=device))
    with torch.no_grad():
...
def evaluate(root_dir, args, device,isVAE = False):
    x_dir = "../testCartoon/"
    y_dir = "../testHuman/"
    x, x_names, y, y_names = load_x_y(args,x_dir, y_dir)
    if isVAE:
        feature_node_x_y = gen_images_VAE(root_dir,x,y,x_names, y_names, args, device,True)
        desired = x_names.index("4.png")
    else:
        feature_node_x_y = gen_images(root_dir,x,y,x_names, y_names, args, device,True)
