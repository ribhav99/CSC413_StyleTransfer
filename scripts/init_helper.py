...
def get_down_conv_block(input_channel, output_channel, kersize, act_fn, norm_type, tonorm=True, stride=2):
    ...
    layer.append(nn.Conv2d(input_channel, output_channel,
                           kernel_size=kersize, stride=stride, padding=to_pad))
    if tonorm:
...
def gen_downconv(channel_list, act_fn, norm_type):
    ...
    for i in range(len(channel_list) - 1):
        ...
        layer += get_down_conv_block(input_channel,
                                     output_channel, 4, act_fn, norm_type)
    return layer
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channel, norm_type, dropout=False):
        ...
        self.block1 = nn.Sequential(
            *get_res_block(in_channel, norm_type, dropout))
        self.block2 = nn.Sequential(
            *get_res_block(in_channel, norm_type, False))
...
def get_up_conv_block(input_channel, output_channel, kersize, act_fn, norm_type, tonorm=True, dropout=False, conv2T=False):
    ...
    if conv2T:
        layer.append(nn.ConvTranspose2d(input_channel, output_channel,
                                        kernel_size=kersize, stride=2, padding=to_pad))
    else:
        layer += [nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False), nn.ReflectionPad2d(
            1), nn.Conv2d(input_channel, output_channel, kernel_size=3, stride=1, padding=0)]
    if tonorm:
        layer.append(get_norm(output_channel, norm_type))
    if dropout:
...
def gen_upconv(channel_list, act_fn, norm_type, last_act_fn='tanh', dropout=False, conv2T=False):
    ...
    for i in range(len(channel_list) - 1):
        ...
        if i == (len(channel_list) - 2):
            act_fn = last_act_fn
        layer += get_up_conv_block(prev_channel, target_channel,
                                   4, act_fn, norm_type, dropout=dropout,  conv2T=conv2T)
    return layer
